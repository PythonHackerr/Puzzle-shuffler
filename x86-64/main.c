...
{
    int column = block_number % 4;
    int row = block_number / 4;

    return info->pImg + row * info->line_bytes * 80 + column * 3 * 80;
}
...
void shuffle(ImageInfo *ptr)
{
    ...
    for (int i = 0; i < 12; ++i)
    {
        char *a = get_block_start_ptr(i, ptr);
        char *b = get_block_start_ptr(puzzle[i], ptr);
        swap_blocks(a, b, ptr);
    }
}
